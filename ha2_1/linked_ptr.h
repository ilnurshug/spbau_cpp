...
public:
    ...
    {
        if (obj_ == rhs.obj_) return *this;
        
        linked_ptr<T> tmp(rhs);
        swap(tmp, *this);
    ...
    {
        if (lhs.obj_ == rhs.obj_) return;
        
        if (lhs.prev_) lhs.prev_->nxt_ = &rhs;
        if (lhs.nxt_)  lhs.nxt_->prev_ = &rhs;
        if (rhs.prev_) rhs.prev_->nxt_ = &lhs;
        if (rhs.nxt_)  rhs.nxt_->prev_ = &lhs;
