...
std::string const *rvalue_const(std::string &&s) {
    return nullptr;
}
...
void f(S const & s)
{
    // single argument copy should be stored inside bind
    assert(s.copied == 1);
    // "const &" reference should be forwarded until place of copying
    assert(s.moved == 0);
}
...
int main() {
    ...
    {
        int x = 0;
        n::bind(&assign_five, x)();
        assert(x == 0);
    }

    {
        //n::bind(&fv); // shouldn't compile
    }
