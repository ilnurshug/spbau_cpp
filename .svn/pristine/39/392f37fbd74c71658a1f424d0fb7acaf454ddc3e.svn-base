...
void test_rvalue_argument()
{
    n::function<void (std::string &&)> f = [](std::string &&) {};
    f(std::string("test"));
    f("test");
}

struct copy_counter_t
{
    copy_counter_t()
            : counter(0)
    {
    ...
    copy_counter_t(copy_counter_t const & other)
...
void test_copying()
{
    n::function<std::size_t(copy_counter_t)> f =
            [](copy_counter_t const & cc)
            {
                return cc.counter;
            };
    assert(f(copy_counter_t()) >= 1);
}
...
//        int (*foo1)(int);
...

    {
        ...
        static_assert(std::is_same<std::result_of<n::function<int const()>()>::type, int>::value, "");
        assert(f4() == 31);
        ...
        static_assert(std::is_same<std::result_of<n::function<std::string const * const()>()>::type, std::string const *>::value, "");
